...
/// </para>
/// </remarks>
public sealed class StorageProviderResolver
{
    private readonly Dictionary<string, IBlazorSqliteStorageProvider> _providers;
    private readonly IStorageBindingStore _bindingStore;
    ...
    public StorageProviderResolver(
        ...
        ILogger<StorageProviderResolver>? logger = null)
    {
        ArgumentNullException.ThrowIfNull(providers);
        ArgumentNullException.ThrowIfNull(bindingStore);

        _bindingStore = bindingStore;
        _logger = logger ?? NullLogger<StorageProviderResolver>.Instance;
    ...
    /// </exception>
    public async ValueTask<StorageResolution> ResolveAsync(
        string databaseName,
    ...
    private async ValueTask<StorageResolution> ResolveExistingDatabaseAsync(
        string databaseName,
        ...
        CancellationToken cancellationToken)
    {
            ...
            throw new BlazorSqliteStorageUnavailableException(
                ...
                [
                    new StorageCandidateOutcome
                    {
                        ...
                        Status = StorageCandidateStatus.NotRegistered,
                        Explanation =
        ...
        return new StorageResolution
        {
            ...
            [
                ...
                {
                    ProviderName = bound.Name,
                    Status = StorageCandidateStatus.Selected,
                    Probe = probe,
    ...
    private async ValueTask<StorageResolution> ResolveNewDatabaseAsync(
        string databaseName,
        BlazorSqliteStorageSelection selection,
        CancellationToken cancellationToken)
    {
        var attempts = new List<StorageCandidateOutcome>(selection.Candidates.Count);
        ...
        for (var index = 0; index < selection.Candidates.Count; index++)
        {
            ...
            {
                attempts.Add(new StorageCandidateOutcome
                {
                ...
                && !selection.AllowNonPersistentFallback)
            {
                ...
                {
                    ProviderName = candidateName,
                    Status = StorageCandidateStatus.RejectedAsNonPersistent,
                    Probe = probe,
                    Explanation =
            ...
            for (var rest = index + 1; rest < selection.Candidates.Count; rest++)
            {
                attempts.Add(new StorageCandidateOutcome
                {
                    ...
                    Status = StorageCandidateStatus.NotProbed,
