...
public static class BlazorSqliteLiveQueryExtensions
{
    /// <summary>
    /// Re-executes <paramref name="query"/> whenever a table it reads is written.
    /// </summary>
    public static IBlazorSqliteLiveQuery<IReadOnlyList<T>> AsLiveQuery<T>(this IQueryable<T> query)
    {
    ...
    /// Re-executes <paramref name="query"/> whenever a table it reads is written.
    /// </summary>
    public static IBlazorSqliteLiveQuery<IReadOnlyList<T>> AsLiveQuery<T>(
        ...
        BlazorSqliteConnection connection)
    {
        ...
        var tables = BlazorSqliteTableNames.Extract(query.ToQueryString());
        ...
        var gate = context is null ? null : new BlazorSqliteSaveChangesGate(context);

        return new BlazorSqliteLiveQuery<IReadOnlyList<T>>(
            connection,
            async ct =>
            {
                if (gate is not null)
