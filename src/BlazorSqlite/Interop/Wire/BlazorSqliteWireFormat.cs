...
/// </para>
/// </remarks>
public static class BlazorSqliteWireFormat
{
    ...
    public static BlazorSqliteWireRequest[] EncodeBatch(IReadOnlyList<BlazorSqliteCommandRequest> batch)
    {
        ArgumentNullException.ThrowIfNull(batch);

        var encoded = new BlazorSqliteWireRequest[batch.Count];
    ...
    private static BlazorSqliteWireRequest EncodeRequest(BlazorSqliteCommandRequest request)
    {
        var parameters = new BlazorSqliteWireParameter[request.Parameters.Count];
        for (var i = 0; i < request.Parameters.Count; i++)
        {
            ...
            parameters[i] = new BlazorSqliteWireParameter(parameter.Name, type, value);
        }

        return new BlazorSqliteWireRequest(request.CommandText, EncodeResultKind(request.ResultKind), parameters);
    }

    private static string EncodeResultKind(BlazorSqliteResultKind kind) => kind switch
    {
        BlazorSqliteResultKind.NonQuery => "nonQuery",
        BlazorSqliteResultKind.Scalar => "scalar",
        BlazorSqliteResultKind.Reader => "reader",
    ...
    /// disagreement with the ADO.NET binder that owns the decision, so they are rejected with a
    /// message saying so. <see cref="BlazorSqliteParameterBinding"/> converts them before a
    /// parameter reaches the transport.
    /// </remarks>
    ...
    public static IReadOnlyList<BlazorSqliteCommandResult> DecodeResults(JsonElement json)
    {
        ...
        var results = new List<BlazorSqliteCommandResult>(json.GetArrayLength());
    ...
    private static BlazorSqliteCommandResult DecodeResult(JsonElement element)
    {
