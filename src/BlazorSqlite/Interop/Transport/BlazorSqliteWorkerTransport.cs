...
/// <summary>
/// An <see cref="IBlazorSqliteTransport"/> that drives the browser worker over Blazor JS interop.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// Values cross as the tagged JSON described by <see cref="BlazorSqliteWireFormat"/>. The ADO.NET
/// layer never sees that encoding, so a cheaper marshaller can replace it later without touching
/// <see cref="BlazorSqliteConnection"/>.
/// </para>
/// </remarks>
public sealed class BlazorSqliteWorkerTransport : IBlazorSqliteTransport
{
    ...
    private readonly BlazorSqliteWorkerTransportOptions _options;
    private IJSObjectReference? _module;
    private IJSObjectReference? _host;
    private DotNetObjectReference<BlazorSqliteWorkerTransport>? _self;
    ...
    public BlazorSqliteWorkerTransport(IJSRuntime js, BlazorSqliteWorkerTransportOptions options)
    {
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<BlazorSqliteCommandResult>> ExecuteAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await CallAsync(
            new { kind = "execute", batch = BlazorSqliteWireFormat.EncodeBatch(batch) },
            cancellationToken).ConfigureAwait(false);

        return BlazorSqliteWireFormat.DecodeResults(result);
    }
    ...
    private async Task<JsonElement> CallAsync(object request, CancellationToken cancellationToken)
    {
        ...
        return BlazorSqliteWireFormat.DecodeCall(envelope);
    }

    private static string BuildVersionedHostModuleUrl()
    {
        var assembly = typeof(BlazorSqliteWorkerTransport).Assembly;
