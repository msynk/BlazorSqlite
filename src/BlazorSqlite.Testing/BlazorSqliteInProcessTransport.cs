...
/// <summary>
/// An <see cref="IBlazorSqliteTransport"/> that runs SQLite in-process via Microsoft.Data.Sqlite.
/// </summary>
/// <remarks>
...
/// <see cref="BlazorSqliteFunctions"/>), and reporting committed writes through the same
/// update-hook and commit-hook mechanism - so a test that passes here is a meaningful signal.
/// </remarks>
public sealed class BlazorSqliteInProcessTransport : IBlazorSqliteTransport
{
    ...
    /// </param>
    public BlazorSqliteInProcessTransport(
    ...
    public Task OpenAsync(string databaseName, CancellationToken cancellationToken = default)
    {
        ...
        {
            BlazorSqliteFunctions.Register(_connection);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var results = new List<BlazorSqliteCommandResult>(batch.Count);
        try
        {
        ...
        return Task.FromResult<IReadOnlyList<BlazorSqliteCommandResult>>(results);
    }
    ...
    /// </remarks>
    private void NotifyIfCommitted(SqliteConnection connection, IReadOnlyList<BlazorSqliteCommandRequest> batch)
    {
        foreach (var request in batch)
        {
            if (BlazorSqliteTableNames.LooksLikeWrite(request.CommandText))
            {
                _writtenTables.UnionWith(BlazorSqliteTableNames.Extract(request.CommandText));
            }
        }
    ...
    private static BlazorSqliteCommandResult Execute(SqliteConnection connection, BlazorSqliteCommandRequest request)
    {
        ...
        {
            case BlazorSqliteResultKind.NonQuery:
                return new BlazorSqliteCommandResult { RecordsAffected = command.ExecuteNonQuery() };

            case BlazorSqliteResultKind.Scalar:
            {
                var value = command.ExecuteScalar();
                return new BlazorSqliteCommandResult
                {
                    ColumnNames = ["value"],
                    ColumnTypes = [null],
            ...
            case BlazorSqliteResultKind.Reader:
    ...
    private static BlazorSqliteCommandResult Materialize(SqliteDataReader reader)
    {
